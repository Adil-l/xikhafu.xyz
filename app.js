...
(() => {
  ...
  async function hydrateAdminOperational(pin=cloudCredentials.adminPin){
    try{let cloud=await cloudRpc("load_admin_operational_state",{p_admin_pin:pin});if(!cloud)return false;if(Number(state.settings.operationalMigrationVersion||0)<1){ensureOperationalKeys();const localOrders=state.orders.slice(),localRecharges=state.recharges.slice(),localDonations=state.donationPledges.slice(),localSettings=operationalSettingsPayload(),localBalances=new Map(state.users.map(u=>[Number(u.id),Number(u.monthlyBalance||0)]));(cloud.users||[]).forEach(mergeCloudUser);mergeCloudSettings(cloud.settings);(cloud.users||[]).forEach(remote=>{const oldBalance=localBalances.get(Number(remote.id))||0;if(Number(remote.monthlyBalance||0)===0&&oldBalance>0)user(remote.id).monthlyBalance=oldBalance});const mergeRows=(local,remote)=>{const rows=new Map();local.forEach(row=>rows.set(row.syncKey,row));remote.forEach(row=>rows.set(row.syncKey,row));return [...rows.values()]};state.orders=mergeRows(localOrders,cloud.orders||[]);state.recharges=mergeRows(localRecharges,cloud.recharges||[]);state.donationPledges=mergeRows(localDonations,cloud.donations||[]);Object.assign(state.settings,localSettings);await syncAdminOperational();state.settings.operationalMigrationVersion=1;save();cloud=await cloudRpc("load_admin_operational_state",{p_admin_pin:pin})||cloud}(cloud.users||[]).forEach(mergeCloudUser);mergeCloudSettings(cloud.settings);state.orders=cloud.orders||[];state.recharges=cloud.recharges||[];state.donationPledges=cloud.donations||[];ensureOperationalKeys();save();return true}catch{return false}
  }
